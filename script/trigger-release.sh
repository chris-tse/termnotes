...
if [ "$minor" = true ]; then
    ...
    let "minor_number++"
    next_version="${VERSION[0]}.$minor_number.0"
else
    # Increment the patch version
    patch_number=${VERSION[2]}
    let "patch_number++"
    next_version="${VERSION[0]}.${VERSION[1]}.$patch_number"
fi
...
gh workflow run publish.yml -f version="$next_version"

echo "✅ Workflow triggered successfully!"
echo "📋 Check status: https://github.com/chris-tse/termnotes/actions"
